...
busco_manatee <- ggplot(busco, aes(x = Approach, y= n, fill=as.factor(busco_status)))+
  geom_bar(stat = "identity", position = "stack",color="black",linewidth=.25) +
...
isoforms_ncbi_c$Type <- "TriManLat1.0 annotation"
...
isoforms_ncbi_percent$Type <- as.factor("TriManLat1.0 annotation")
...
class_ncbi_gge <- ggplot(sqanti_df, aes(fill=structural_category, y=n, x=type)) + 
  ...
  scale_x_discrete(labels=c("LRB vs TriManLat1.0", "TriManLat1.0 vs LRB"))+labs(x = "", y = "Nº of isoforms", fill = "Structural Category")+
...
```{r}
#Guardamos el plot:
ggsave("C:/Users/Usuario/Desktop/Manatee/Comparison_annotations_ncbi_gge_number.jpg", class_ncbi_gge, device = "jpeg", width=12.5, height=7,limitsize = FALSE, dpi=300)
ggsave("C:/Users/Usuario/Desktop/Manatee/Comparison_annotations_ncbi_gge_number.pdf", class_ncbi_gge, device = "pdf", width=12.5, height=7,limitsize = FALSE, dpi=300)
```
...
class_ncbi_gge_perc <- ggplot(sqanti_df_perc, aes(fill=structural_category, y=percent, x=type)) +
  scale_x_discrete(labels=c("LRB  vs TriManLat1.0", "TriManLat1.0 vs LRB")) +
...
```{r}
#Guardamos el plot:
ggsave("Comparison_annotations_ncbi_gge_percentage.jpg", class_ncbi_gge_perc, device = "jpeg", width=12.5, height=7,limitsize = FALSE, dpi=300)
ggsave("Comparison_annotations_ncbi_gge_percentage.pdf", class_ncbi_gge_perc, device = "pdf", width=12.5, height=7,limitsize = FALSE, dpi=300)
```
...
```{r}
class_isoforms <- ggplot(data_isoforms, aes(fill=num_cat, x=Type, y=percent)) +geom_bar(position="stack", stat="identity", color= "black", width = 0.7, size=.25)+labs(x = "Annotation", y = "Genes (%)", fill = "Isoforms per gene")+theme_classic()+theme(mytheme)+theme(axis.text.x = element_text(size=10))+theme(legend.position = "bottom")+scale_fill_manual(values=color_isof_num) + theme(legend.key.size = unit(.5,"line")) + scale_x_discrete(labels=c("TriManLat1.0", "LRB"))
...
```{r}
#Guardamos el plot:
ggsave("Number_isoforms_per_gene.jpg", class_isoforms, device = "jpeg", width=12.5, height=7,limitsize = FALSE, dpi=300)
ggsave("Number_isoforms_per_gene.pdf", class_isoforms, device = "pdf", width=12.5, height=7,limitsize = FALSE, dpi=300)
```
...
```{r}
#Guardamos el plot:
ggsave("Number_exon_fusion_vs_no_fusion.jpg", p, device = "jpeg", width=12.5, height=7,limitsize = FALSE, dpi=300)
ggsave("Number_exon_fusion_vs_no_fusion.pdf", p, device = "pdf", width=12.5, height=7,limitsize = FALSE, dpi=300)
```
...
```{r}
#Guardamos el plot:
ggsave("Length_fusion_vs_no_fusion.jpg", p2, device = "jpeg", width=12.5, height=7,limitsize = FALSE, dpi=300)
ggsave("Length_fusion_vs_no_fusion.pdf", p2, device = "pdf", width=12.5, height=7,limitsize = FALSE, dpi=300)
``` 
