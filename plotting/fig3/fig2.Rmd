...
plot_g_ai <- ggplot(conjunto_gene_ab_initio, aes(x = as.factor(Approach), y= value, fill=metric))+
  ...
  ylim(0,50)+
  theme(mytheme)+
  scale_fill_manual(values =  c("#FDC659", "#15918A", "#F58A53")) + theme(legend.key.size = unit(0.2, "cm"))+theme(axis.text.y = element_text(face = "italic")) +
  coord_flip() 
...
plot_g_ed <-ggplot(conjunto_gene_ed, aes(x = Technology, y= value, fill=as.factor(metric)))+
  geom_bar(stat = "identity", position = "dodge",color="black", width = 0.60, linewidth=  .25)+
  theme_classic()+labs(x = "", y = "Value (%)", fill = "Metric")+
  ylim(0,50)+theme(mytheme)+theme(plot.title = element_text(hjust = 0.5, face="italic"))+
  scale_fill_manual(values =  c("#FDC659", "#15918A", "#F58A53"))+
  coord_flip()
...
saturation_curve <- ggplot(stats_gene_final)+
  ...
  labs(x="Subsampling of reads (%)",y="Score (%)")+theme_classic()+ scale_x_continuous(breaks=c(25,50,75,100))+ylim(25,53)+scale_color_manual(values=colores_graficos )+theme(mytheme)+
...
ml <- ggplot(data=missed_loci, aes(x=Subsampling, y=Missed_loci, group=Approach)) +
  ...
  labs(x = "Subsampling of reads (%)", y = "Missed loci (%)", color = "Approach")+theme_classic()+theme(mytheme)+theme(legend.position = "bottom")+scale_color_manual(values=colores_graficos) + theme(
...
ml_no_margin <- ggplot(data=missed_loci, aes(x=Subsampling, y=Missed_loci, group=Approach)) +
  geom_line(size=1, aes(linetype= Approach))+
  geom_point(aes(x=Subsampling,y=Missed_loci)) +
  labs(x = "Subsampling of reads (%)", y = "Missed loci (%)", color = "Approach")+theme_classic()+theme(mytheme)+theme(legend.position = "bottom")+scale_color_manual(values=colores_graficos) 
...
ggsave("Figura_3_complete_gr.jpg", device = "jpeg", width=7.5, height=6.55,limitsize = FALSE, dpi=300)
